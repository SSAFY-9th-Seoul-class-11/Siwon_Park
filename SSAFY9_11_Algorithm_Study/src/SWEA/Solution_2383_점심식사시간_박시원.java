...
public class Solution_2383_점심식사시간_박시원 {
	static int N, cnt, ans;
	static final int M = 1, W = 2, D = 3, C = 4; //이동 중, 대기 중, 내려가는 중, 완료
	...
	static int processDown(ArrayList<Person> list, int height) {
		...
		int ingCnt = 0, cCnt = 0; // 내려가는 사람 수, 완료 사람수
		Person p = null;
		while(true) {
			for(int i = 0; i<size; i++) {
				...
				else if(p.arrivalTime == time) p.status = W;
